...
var Simbla = (function () {
    ...
    _simbla.Query.prototype.aggregate = function (groupby, group, groupcomplex) {
        ...
        if (Object.keys(group).length === 0) return err("group object must countain values");
        ...
        var d = {
            ...
            limit: limit,
            order: this._order,
            // todo add timeZone for date agrregation when server is ready to support it
            // timeZone: timeZone,
        };
    ...
    if (fileInputs.length > 0) {
        ...
        fileInputs.each(function () {
            ...
            if (input.files.length > 0) {
                ...
                promises.push(p);
            } else if ($(input).is(':visible') && obj.get(input.name)) {
...
function simblaTable() {
    ...
    function query() {
        ...
        if (!m.data('is-aggregate'))
            return query.find().then(resultsFunc).catch(error);

        query.limit(10000);
        return getAggregateQuery(query).then(function (res) {
            return resultsFunc(res.results);
        }).catch(error);
    }
    ...
    for (var i in o.attributes) {
        ...
        if (input.is('[type="date"],[type="datetime-local"]') && val) {
            ...
            if (Array.isArray(val)) {
                ...
                if (key)
                    args.push(key)
                input.val.apply(input, args).data('val', val.id).trigger('change', {
                    validData: true
                });
            }
...
function autoRules() {
    ...
    function testAndRun(runOnRules, d) {
        ...
        try {
            ...
            failedRules.forEach(function (r) {
                ...
                console.log('apply rule', r.name, 'status', false, r);

            });
            successRules.forEach(function (r) {
                applyActions(r.actions, eventCount + 1);
                console.log('apply rule', r.name, 'status', true, r);
    ...
    form.on('simblaobjectsettoform', function () {
...
$(function () {
    ...
    $(document).on('click', '.db-form-add', function () {
        ...
        ctrl.find('input[type="file"]').show();
        ctrl.find('input[type="file"]')[0].value = "";
...
$(document).on('change', 'select.select-pointer', function (e, d) {
    var T = $(this);
    if (!T.val() || T.prop('multiple') || !T.attr('targetclass') || T.val().length !== 10) return;
